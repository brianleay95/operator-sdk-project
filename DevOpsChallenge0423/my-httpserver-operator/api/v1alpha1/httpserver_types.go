...
type HTTPServerSpec struct {
	// Number of replicas for the HTTP server
	Replicas int32 `json:"replicas"`

	// Container image for the HTTP server
...
// HTTPServer is the Schema for the httpservers API
type HTTPServer struct {
	metav1.TypeMeta   `json:",inline"`
	metav1.ObjectMeta `json:"metadata,omitempty"`

	Spec   HTTPServerSpec   `json:"spec,omitempty"`
	Status HTTPServerStatus `json:"status,omitempty"`
}
